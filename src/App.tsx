...
export default function App() {
  ...
  const handleGeneratePodcast = async () => {
    if (!file) return;
    setIsPodcastLoading(true); setStatus('Step 1/2: Generating podcast script...');
    setQuizData(null); setPodcastScript(null); setAudioUrl(null);
    ...
    try {
      ...
      // 2. Get the Audio
      setStatus('Step 2/2: Converting script to audio...');
      ...
      if (!audioRes.ok) throw new Error(audioData.error || "Failed to generate audio on backend");
      if (!audioData.audioData) throw new Error("Audio generation timed out.");
      ...
      if (url) {
        ...
        throw new Error("Failed to decode audio properly.");
      }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-6 md:p-8 mb-8">
          ...
          <div className="flex gap-4">
            <button onClick={handleGenerateQuiz} disabled={isQuizLoading || isPodcastLoading || !file} className="flex-1 bg-blue-600 text-white p-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 disabled:opacity-50">
              {isQuizLoading ? <Loader2 className="animate-spin" /> : <BookOpen size={20}/>} Generate Quiz
            </button>
            <button onClick={handleGeneratePodcast} disabled={isQuizLoading || isPodcastLoading || !file} className="flex-1 bg-emerald-600 text-white p-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 disabled:opacity-50">
              ...
              {history.map((item) => (
                <button key={item.id} onClick={() => { if (item.type === 'quiz') { setQuizData(item.data); setPodcastScript(null); setAudioUrl(null); setQuizSubmitted(true); } else { setPodcastScript(item.data.script); setAudioUrl(item.data.audioUrl); setQuizData(null); }}} className="w-full flex items-center justify-between p-3 bg-white border rounded-lg hover:border-blue-500 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left"><div className="text-sm font-bold text-slate-700">{item.filename}</div><div className="text-[10px] text-slate-400">{item.date}</div></div>
                  </div>
          ...
          <div className="bg-white p-8 rounded-2xl border shadow-sm space-y-8 animate-in fade-in duration-500">
                  ...
                  {Object.entries(q.options).map(([k, v]: any) => {
                    ...
                    return (
                      <button 
                        ...
                        className={cn(
                          "w-full text-left p-4 border rounded-xl transition-all font-medium",
                          !quizSubmitted && isSelected ? "border-blue-500 bg-blue-50 shadow-sm" : "border-slate-200 hover:bg-slate-50",
                          quizSubmitted && isCorrect ? "border-emerald-500 bg-emerald-50 text-emerald-800" : "",
                          quizSubmitted && isSelected && !isCorrect ? "border-red-500 bg-red-50 text-red-800" : ""
                        )}
                      >
            ...
            {!quizSubmitted && (
              <button 
                ...
                className="w-full py-4 mt-6 bg-slate-900 text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition-all transform active:scale-95"
              >
                Submit Quiz
              </button>
            )}
          ...
          <div className="bg-white p-8 rounded-2xl border shadow-sm space-y-6 animate-in fade-in duration-500">
            ...
            {audioUrl ? (
              ...
              <div className="p-4 bg-amber-50 text-amber-800 rounded-lg text-sm border border-amber-200">
                ⚠️ Audio generation failed or timed out. You can still read the script below!
              </div>
            )}
